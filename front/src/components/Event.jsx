...
const Event = ({ onClose, startDate, endDate, eventDetails, onDelete, categories, prof }) => {
  ...
  useEffect(() => {
    ...
    console.log(updatedNotification + " " + id);
    if (!updatedNotification && id != 0 && !no) {
      setNo(true);
      axiosInstance.get(`api/events/${id}/notifications/1`).then((res) => {
    ...
    if (notification) {
      if (isNaN(notification.hoursBefore)) {
        ...
        if (!updatedNotification) {
          ...
          axiosInstance.put(`api/notifications/${updatedNotification.id}`, {
            title: notification.title, description: notification.description,
            slug: notification.slug, hoursBefore: notification.hoursBefore, event_id: eventID
          }).then((res) => {
            console.log(res.data);
  ...
  const updateEventCategory = (id, name) => {
    setEventCategory({id:id, name:name});
  };
          ...
          {eventDetails ? (
            ...
            <div>
              ...
              <select
                className="event-input"
                value={eventCategory.id}
                onChange={(e) => updateEventCategory(e.target.value, e.target.options[e.target.selectedIndex].text)}
              >
                <option value="">Select a category</option>
                {categories.map((category) => (
                  <option key={category.id} value={category.id}>
